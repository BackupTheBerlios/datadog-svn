...
public class TransportPacket {
  ...
  public int adaptation_extension_flags() {
    if (!hasAdaptationExtension()) return 0;
   ...
   */
  public long PCR() {
    if ((adaptation_flags() & adapflag_PCR_flag) == 0) return 0;
   ...
   */
  public long OPCR() {
    if ((adaptation_flags() & adapflag_OPCR_flag) == 0) return 0;
  ...
  public int splice_countdown() {
    if ((adaptation_flags() & adapflag_splicing_point_flag) == 0) return 0;
  ...
  public byte[] private_data() {
    if ((adaptation_flags() & adapflag_transport_private_data_flag) == 0) return new byte[0];
  ...
  public int ltw_offset() {
    if ((adaptation_extension_flags() & adapextenflag_ltw_flag) == 0) return 0;
    int pos = adaptationExtensionOffset(0);

    if ((data[pos+0] & 0x80) == 0) return -1; // i.e. not valid
  ...
  public int piecewise_rate() {
    if ((adaptation_extension_flags() & adapextenflag_piecewise_rate_flag) == 0) return 0;
  ...
  public int seamless_splice_type() {
    if ((adaptation_extension_flags() & adapextenflag_seamless_splice_flag) == 0) return 0;
  ...
  public byte[] reserved_bytes() {
    if (!hasAdaptationExtension()) return new byte[0];
  ...
  public byte[] stuffing_bytes() {
    if (!hasAdaptation()) return new byte[0];
    
    int pos = adaptationOffset(adapflag_PCR_flag|adapflag_OPCR_flag|adapflag_splicing_point_flag|adapflag_transport_private_data_flag|adapflag_adaptation_field_extension_flag);
   ...
   * 
   * @param payloadOff Offset into the payload.
   * @param dest Buffer to put the data in.
   ...
   * 
   * @param flags Bitmask of the values to skip if present. 
   * @return Position of the field after those skipped.
   */
  private int adaptationOffset(int flags) {
    
    if (!hasAdaptation()) throw new RuntimeException("No adaptation field present");
 
    flags &= adaptation_flags();

    int pos = 4 + 1 + 1;
    if ((flags & adapflag_PCR_flag) != 0) pos += 6;
    if ((flags & adapflag_OPCR_flag) != 0) pos += 6;
    if ((flags & adapflag_splicing_point_flag) != 0) pos += 1;
  ...
  private int adaptationExtensionOffset(int flags) {
    if (!hasAdaptation()) throw new RuntimeException("No adaptation field present");
    if ((adaptation_flags() & adapflag_adaptation_field_extension_flag) == 0) throw new RuntimeException("No adaptation extension field present");
    ...
    if ((flags & adapextenflag_ltw_flag) != 0) pos += 2;
    if ((flags & adapextenflag_piecewise_rate_flag) != 0) pos += 3;
    if ((flags & adapextenflag_seamless_splice_flag) != 0) pos += 5;
